...
def get_model(model_name, config, graph, pred_layer, device):
    ...
    print(f"Loading model weights from {ROOT_DIR / 'saved_models' / f'{model_name}.pt'}")
...
def predict_custom(model_name, csv_file, node_js, graph, model, output_dir):
    ...
    out_path = output_dir / f'{model_name}_{csv_file.split(".")[-2].split("/")[-1]}_predictions_{datetime.now().strftime("%Y%m%d_%H%M%S")}.tsv'
    df.to_csv(out_path, sep="\t", index=False)
    print(f"Saved predictions to {out_path}")

...
def run_model(model,
    ...
    # Save test results
    with open(fr'{output_dir}/{model_name}_test_results_{datetime.now().strftime("%Y%m%d_%H%M%S")}.json', 'w') as f:
    ...
    if save_predictions:       
        ...
        df_preds.to_csv(fr"{output_dir}/{model_name}_test_predictions_{datetime.now().strftime('%Y%m%d_%H%M%S')}.tsv", sep="\t", index=None)
...
def main():
    ...
    crossbar_kg = torch.load(ROOT_DIR / fr"data/crossbar_kg/crossbar-kg_{args.compound_representation}.pt")   
    ...
    tr_ts_data = torch.load(ROOT_DIR / fr"data/train_test_samples/{args.split}_train_test_edges_{args.compound_representation}.pt")
    
    tr_edge_index, tr_edge_label = tr_ts_data["train_data"], tr_ts_data["train_label"]
    ts_edge_index, ts_edge_label = tr_ts_data["test_data"], tr_ts_data["test_label"]
   ...
   # Start training
    ...
    run_model(model, train_loader, test_loader, model_cfg, optimizer, device, model_name, tr_pchembl_median, edge_name, args.output_dir, args.save_model, args.save_predictions)
